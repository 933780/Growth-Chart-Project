    ...
    PView.prototype = {
        ...
        _getHeuristics : function() {
            var out = {},
                ...
                // WHO/CDC percentile cutoffs (IAP uses adult-equivalent BMI values instead)
                cutoffOverweight = 85,
                cutoffObese      = 95,
                // IAP adult-equivalent BMI cutoffs: Z-scores derived from BMI 23 and 27
                // at age 18 per IAP 2015 revised growth charts (Indian Pediatrics, Vol 52)
                IAP_Z_OVERWEIGHT = PATIENT.gender === "male" ? 0.67322324 : 0.69316609,
                IAP_Z_OBESE      = PATIENT.gender === "male" ? 2.14230276 : 2.05538545;
            ...
            if (!lastWeightEntry || !lastHeightEntry) {
                ...
                if (useIAP && lastBMIEntry && lastBMIEntry.agemos >= 60) {
                    // IAP uses adult-equivalent BMI cutoffs (23=overweight, 27=obese),
                    // not fixed percentiles. Compare raw BMI against age-specific
                    // cutoff values derived from the IAP LMS data using the Z-scores
                    // anchored at those adult BMI values at age 18.
                    var bmiDataSetIAP = GC.getDataSet("IAP", "BMI", PATIENT.gender, 0, lastBMIEntry.agemos);
                    if (bmiDataSetIAP) {
                        var bmiCutoffOverweight = GC.findXFromZ(IAP_Z_OVERWEIGHT, bmiDataSetIAP, PATIENT.gender, lastBMIEntry.agemos);
                        var bmiCutoffObese      = GC.findXFromZ(IAP_Z_OBESE,      bmiDataSetIAP, PATIENT.gender, lastBMIEntry.agemos);
                        var bmi = lastBMIEntry.bmi;
                        if (!isNaN(bmiCutoffOverweight) && !isNaN(bmiCutoffObese)) {
                            // Map to synthetic percentile values that feed the existing
                            // cutoffOverweight/cutoffObese comparison logic below
                            if (bmi >= bmiCutoffObese) {
                                obesity = 96; // above obese cutoff
                            } else if (bmi >= bmiCutoffOverweight) {
                                obesity = 86; // between overweight and obese cutoff
                            } else {
                                obesity = GC.findPercentileFromX(bmi, bmiDataSetIAP, PATIENT.gender, lastBMIEntry.agemos) * 100;
                            }
                        } else {
                ...
                else if (lastBMIEntry && lastBMIEntry.agemos >= 24) {
                    // WHO path — standard percentile-based classification
                ...
                healthyWeightMax = GC.findXFromPercentile(
                    useIAP ? Math.normsdist(IAP_Z_OVERWEIGHT) : 0.85,
                    dataSet,
                    PATIENT.gender,
                    lastWeightEntry.agemos
                );
