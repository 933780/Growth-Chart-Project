...
GC.get_data = function() {
    ...
    // ─── LOCAL BYPASS (no SMART server needed) ────────────────────────────────
    // Used when Angular form passes patient data via URL param or localStorage.
    // Falls back to normal SMART OAuth if no local data found.

    function buildClientFromLocalStorage(bundle) {
        var patient = bundle.entry.find(function(e) {
            return e.resource.resourceType === "Patient";
        }).resource;

        var observations = bundle.entry
            .filter(function(e) { return e.resource.resourceType === "Observation"; })
            .map(function(e) { return e.resource; });

        return {
            patient: {
                id: patient.id,
                read: function() { return Promise.resolve(patient); }
            },
            request: function(url) {
                if (url.indexOf("FamilyMemberHistory") !== -1) {
                    return Promise.resolve([]);
                }
                // Return flat array of observations (matches flat:true behaviour)
                return Promise.resolve(observations);
            },
            units: {
                kg:  function(q) { return q.value; },
                cm:  function(q) { return q.value; },
                any: function(q) { return q.value; }
            },
            byCode: function(resources, prop) {
                var ret = {};
                (resources || []).forEach(function(r) {
                    var codings = (r[prop] && r[prop].coding) || [];
                    codings.forEach(function(c) {
                        if (!ret[c.code]) ret[c.code] = [];
                        ret[c.code].push(r);
                    });
                });
                return ret;
            },
            getPath: function() { return undefined; }
        };
    }

    // ─── ENTRY POINT: check URL params ───────────────────────────────────────
    ...
    if (encodedToken) {
        ...
        return fetch(API_BASE_URL + '/api/patients/' + encodeURIComponent(payload.patientId) + '/data')
            ...
            .catch(function(e) {
                console.error("Failed to load from .NET backend:", e);
                throw new Error("Backend error: " + e.message);
            });
    }

    // PATH 2: Legacy URL param bypass — patientData param (whole bundle encoded)
    var localBundle = null;
    try {
        var paramData = urlParams.get("patientData");
        if (paramData) {
            localBundle = JSON.parse(decodeURIComponent(paramData));
        }
        // Fallback: try localStorage (works if same origin)
        if (!localBundle) {
            var raw = localStorage.getItem("patientData");
            if (raw) localBundle = JSON.parse(raw);
        }
    } catch(e) {
        console.warn("Failed to parse local patient data:", e);
    }

    if (localBundle) {
        return onReady(buildClientFromLocalStorage(localBundle));
    }

    // PATH 3: Normal SMART OAuth launch
